...
while True:    
    try:
        # 1. 지금부터 70시간 전 데이터 조회
        query = """
            SELECT "Close", "High", "Low", "Open", "Symbol", "Total_Volume", "time"
            FROM "15m-monitor"
            WHERE time >= now() - interval '70 hour'
            ORDER BY "time"
            """
        ...
        if df is not None and not df.empty:
            ...
            for row in df.itertuples():
                ...
                line_protocol_monitor = (
                    f'1h-monitor,Symbol={row.Symbol} '
                    f'{fields_str} '
                    f'{row.time}'
                )
                ...
                try:
                    ...
                    if response.status_code != 204:
                        ...
                        send_slack_alert(
                            ...
                            summary="1시간봉 데이터 전송에 실패했습니다.",
                            details=error_message,
                            level="danger"
                        )
                    else:
                        logging.info(f"[{time.strftime('%Y-%m-%d %H:%M:%S]')} 1시간봉 데이터 {len(lp_lines_monitor)}건 전송 완료")
